    ...
    public class Monitor
    {
        ...
        public async Task checkSitesAsyn(SiteList site, string siteResponse)
        {
            try
            {
                ...
                siteResponse = siteResponse.Replace("'</body></html>", "").Replace("<html><body>b'", "");
                string jsonData = de_asymetricAbbas(siteResponse);
                ...
                {
                    statuscode = 0;
                    smsError += "##Monitor Service is not working, plz check" +"# now:" +dt.ToString()+  "#Sys:" +sc.Date.ToString()+"#"+ sc.projectname;
                }
                ...
                if (sc.disk == 0 || sc.dbcon == 0 || sc.backup == 0)
                {
                    statuscode = 0;
                    if (sc.disk == 0)
                ...
                await Task.Run(() => st.saveDataAsyn(sc, statuscode, jsonData));
                if (statuscode == 0)
                {
                    string smsText = site.ProjectName + ":" + smsError;
